...
const Country = () => {
  ...
  useEffect(() => {
    const getCountries = async () => {
      ...
      try {
        var datas = await axios.get(`http://bexarehimli-001-site1.htempurl.com/api/Countries/GetAdmin/${page}`, { headers: { "Authorization": token } })
  ...
  return (
    <div className="admin-container">
      <h2 className='text-start  crud-header-entity pb-3' >Country</h2>
      <div className='text-end me-3'>
        <Link className='btn btn-primary' to={"/admin/country/create"}>Create Country</Link>
      </div>
                ...
                countries.countries.map((x, index) => (
                  <tr key={x.id}>
                    ...
                    <td><span className="text-success">
                      <Link to={`/admin/country/edit/${x.id}`} className='btn btn-warning'> <FiEdit className='me-2' />Edit</Link>
                      <button onClick={async () => {
                        var token = "Bearer " + adminToken
                        try {
                          var datas = await axios.delete(`http://bexarehimli-001-site1.htempurl.com/api/Countries/${x.id}`, { headers: { "Authorization": token } })
                          var datas = await axios.get("http://bexarehimli-001-site1.htempurl.com/api/Countries", { headers: { "Authorization": token } })
